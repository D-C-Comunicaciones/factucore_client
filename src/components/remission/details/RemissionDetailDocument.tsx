...
import { StatusBadge } from "@/components/remission/table/columns";
import {
    Table,
    TableBody,
    TableCell,
...
interface RemissionDetailDocumentProps {
    remission: any;
    company: any;
    customer: any;
    items: any[];
...
export function RemissionDetailDocument({
    remission,
    company,
    customer,
    items
}: RemissionDetailDocumentProps) {
    const getNestedValue = (obj: any, key: string) => obj?.[key] ?? obj?.legal_monetary_total?.[key] ?? 0;

    const finalSubtotal = Number(getNestedValue(remission, 'line_extension_amount') || remission.subtotal || 0);
    const finalTotal = Number(getNestedValue(remission, 'payable_amount') || remission.total || 0);
    ...
    const finalTaxes = Number(getNestedValue(remission, 'tax_total') || getNestedValue(remission, 'tax_totals') || remission.tax_amount || taxAmountTotal);
            ...
            >
                ...
                <div className="p-5 sm:p-8 md:p-10">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6 mb-10 border-b pb-8 border-slate-100 text-center md:text-left">
                        ...
                        <div className="w-full md:w-1/3 flex flex-col items-center md:items-end md:pt-2">
                            <div className="text-slate-500 text-xl font-light text-primary">
                                {documentTypeLabel} No. <span className="font-semibold">{remission.prefix || ''}{remission.number || remission.id}</span>
                            </div>
                    ...
                    <div className="mb-10 text-slate-600">
                        <h3 className="font-bold text-slate-800 mb-4">Información de la remisión</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                            <div>
                                <div className="font-bold text-slate-700 mb-1">Cliente</div>
                                <Link href={`/contacts/${customer?.id || remission?.contact_id || ''}`} className="font-medium text-slate-800 underline cursor-pointer hover:bg-slate-100 px-2 py-1 rounded -ml-2 transition-colors inline-block w-fit">
                            ...
                            <div>
                                ...
                                <span>
                                    ...
                                    {(() => {
                                        const dv = customer?.verification_digit ?? remission?.contact?.verification_digit;
                                        return dv != null ? `-${dv}` : '';
                                    })()}
                                </span>
                            </div>
                            <div>
                                ...
                                <span>{remission.created_at || ''}</span>
                            </div>
                            <div>
                                <div className="font-bold text-slate-700 mb-1">Fecha de vencimiento</div>
                                <span>{remission.payment_due_date || remission.expiration_date || remission.created_at || ''}</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <TableBody>
                                ...
                                {resolvedLines.length === 0 && (
                                    <TableRow className="hover:bg-transparent">
                                        <TableCell colSpan={6} className="h-24 text-center text-slate-400 border-l border-r border-gray-200">No hay productos en esta remisión</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
                        ...
                        <div className="w-full md:w-1/2 space-y-10">
                            ...
                            <div className="flex flex-col items-center">
                                <div className="mb-2">
                                    ...
                                    >
                                        {remission.user?.name || 'Administrador'}
                                    </span>
                                </div>
                        ...
                        <div className="w-full md:w-1/2 space-y-4">
                            <div>
                                ...
                                <div className="bg-gray-50 rounded-md p-3 text-xs text-black leading-relaxed min-h-[60px] border border-gray-200 whitespace-pre-wrap">
                                    {remission.notes || remission.observation || '—'}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="bg-gray-50 rounded-md p-3 text-xs text-black leading-relaxed min-h-[60px] border border-gray-200 whitespace-pre-wrap">
                                    {remission.terms_and_conditions || '—'}
                                </div>
                            </div>
                        </div>
                    </div>
