...
interface CuentasPorCobrarData {
  total: number;
  vigentes: { valor: number; documentos: number };
  vencidas: { valor: number; documentos: number };
}

interface CuentasPorCobrarWidgetProps extends CuentasPorCobrarData {}

export function CuentasPorCobrarWidget({ total, vigentes, vencidas }: CuentasPorCobrarWidgetProps) {
  return (
    <div className="h-full flex flex-col justify-between p-1">
      <div>
        <h3 className="text-[11px] font-medium text-gray-600 mb-1 underline">Cuentas por cobrar</h3>
