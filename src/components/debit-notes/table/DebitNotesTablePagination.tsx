...
interface DebitNotesTablePaginationProps {
  page?: number;
  lastPage?: number;
  perPage?: number;
  from?: number;
...
export function DebitNotesTablePagination({
  page = 1,
  lastPage = 1,
  perPage = 10,
  from = 0,
  ...
  onRefresh,
  refreshing = false,
}: DebitNotesTablePaginationProps) {
  ...
  return (
    <div className="flex min-h-12 flex-col gap-2 border-t border-gray-200 px-4 py-2 md:flex-row md:items-center md:justify-between">
      ...
      {/* Derecha: resultados por página + rango + refresh */}
