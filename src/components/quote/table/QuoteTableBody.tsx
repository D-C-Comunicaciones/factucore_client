...
import { QuoteMobileCard } from "@/components/quote/table/QuoteMobileCard";
    ...
    <div className="flex h-64 flex-col items-center justify-center py-8 text-center">
      {showNoDataMessage ? (
        ...
        <>
          ...
          <div className="text-sm text-gray-500 mt-1">
            La búsqueda no arrojó cotizaciones
          </div>
        </>
      )}
    </div>
  ...
  return (
    <div className="relative">
      ...
      <div className="sm:hidden">
        {hasRows ? rows.map((row) => <QuoteMobileCard key={row.id} quote={row.original} />) : stateContent}
      </div>
