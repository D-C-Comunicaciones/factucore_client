...
function ItemRow({
  ...
  const handleItemSelect = (val: string) => {
    ...
    if (selectedOption) {
      ...
      quoteBuilder.updateItem(item.id, "item", ri.name);
      quoteBuilder.updateItem(item.id, "referencia", ri.reference || "");
      ...
      if (ri.tax_rates && ri.tax_rates.length > 0) {
        ...
        quoteBuilder.updateItemTax(item.id, {
        ...
        quoteBuilder.updateItemTax(item.id, null);
      }
      ...
      quoteBuilder.updateItem(item.id, "minimum_stock", warehouseData?.minimum_stock);
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (shouldValidateStock && newQty > currentStock) {
      ...
      quoteBuilder.updateItem(item.id, "cantidad", currentStock > 0 ? currentStock : 0);
    } else {
      quoteBuilder.updateItem(item.id, "cantidad", newQty);
    }
  };
    ...
    >
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => quoteBuilder.updateItem(item.id, "referencia", e.target.value)}
      ...
      <td className="px-2 py-2 align-top">
        <FormattedInput
          placeholder="Precio"
          value={item.precio || 0}
          onChange={(val: number) => quoteBuilder.updateItem(item.id, "precio", val)}
          ...
          onValueChange={(val) => {
            ...
            if (val === "0") {
              ...
              if (tax) {
                ...
                quoteBuilder.updateItemTax(item.id, taxRateObj);
              }
            }
          }}
          options={[
...
export function QuoteItemsTable({
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedItems(quoteBuilder.items.map((i: any) => i.id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const allSelected = quoteBuilder.items.length > 0 && selectedItems.length === quoteBuilder.items.length;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = quoteBuilder.items.findIndex((i: any) => i.id === active.id);
      const newIndex = quoteBuilder.items.findIndex((i: any) => i.id === over?.id);
      if (oldIndex !== -1 && newIndex !== -1) {
        quoteBuilder.reorderItems(oldIndex, newIndex);
      }
    }
  };
          ...
          <tbody>
            <SortableContext items={quoteBuilder.items.map((i: any) => i.id)} strategy={verticalListSortingStrategy}>
              {quoteBuilder.items.length === 0 ? (
                ...
                quoteBuilder.items.map((item: any, index: number) => (
                  <ItemRow
                    key={item.id}
                    item={item}
                    index={index}
