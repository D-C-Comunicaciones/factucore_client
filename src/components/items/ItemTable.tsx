...
import { Pencil, Trash2, X, Lightbulb, LightbulbOff } from "lucide-react";

import { getItemColumns, type Item } from "@/components/items/table/columns";
...
export function ItemTable({
  ...
  const toggleSelectAll = React.useCallback(() => {
    setSelection((prev) => {
      ...
      const areAllSelected =
        items.length > 0 && items.every((i) => prev[String(i.id)]);

      if (areAllSelected) {
        const next: SelectionState = {};
        Object.keys(prev).forEach((key) => {
          if (!visibleIds.has(key)) next[key] = true;
        });
        return next;
      } else {
  ...
  const columns = React.useMemo(
    ...
    [onView, onEdit, onToggleActive, onDelete, toggleSelection, toggleSelectAll, allSelected, someSelected, selection]
  );
  ...
  function handleAddFilter(filterValue: string) {
    setColumnFilters((prev) => {
      if (prev.some((f) => f.id === filterValue)) return prev;
        ...
        <div className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-center shadow-sm">
          ...
          <div className="flex items-center gap-2 ml-4">
            <button
              ...
              onClick={() => console.log("Activar seleccionados")}
            >
            ...
            <button
              ...
              onClick={() => console.log("Desactivar seleccionados")}
            >
            ...
            <button
              className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-bold text-white bg-[#2563eb] hover:bg-[#1d4ed8] rounded-lg transition-colors shadow-sm"
              onClick={() => {
                Object.keys(selection).forEach((id) => onDelete(Number(id)));
                setSelection({});
              }}
            >
