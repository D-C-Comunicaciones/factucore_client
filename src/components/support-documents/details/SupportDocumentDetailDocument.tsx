...
interface SupportDocumentDetailDocumentProps {
    doc: any;
    company: any;
}

export function SupportDocumentDetailDocument({ doc, company }: SupportDocumentDetailDocumentProps) {
    ...
    const lineDiscountsTotal = (doc.discounts || [])
        .filter((d: any) => d.support_document_line_id !== null)
    ...
    const grossSubtotal = Number(doc.subtotal || 0) + lineDiscountsTotal;
    const withholdings = doc.withholdings || [];
    ...
    return (
        <div className="filter drop-shadow-sm">
            ...
            >
                {/* Status ribbon */}
                <div className={`absolute top-6 -left-12 w-48 text-center text-white text-[11px] uppercase tracking-wide font-bold py-1.5 transform -rotate-45 shadow-md ${isCancelled ? 'bg-slate-500' : (!isApproved ? 'bg-orange-500' : (isPaid ? 'bg-green-500' : 'bg-red-500'))}`}>
                    {isCancelled ? 'ANULADO' : (!isApproved ? 'NO APROBADO' : (isPaid ? 'PAGADO' : 'POR PAGAR'))}
                </div>
                ...
                <div className="p-5 sm:p-8 md:p-10">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6 mb-10 border-b pb-8 border-slate-100 text-center md:text-left">
                        ...
                        <div className="w-full md:w-1/3 flex flex-col items-center md:items-end md:pt-2">
                            ...
                            <div className="text-slate-500 text-xl font-light text-primary">
                                No. <span className="font-semibold">{doc.prefix || ''}{doc.number || doc.id}</span>
                            </div>
                            <div className="mt-2 flex items-center justify-center md:justify-end gap-2 uppercase">
                                ...
                                <DianStatusBadge status={doc.dian_status} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 text-slate-600">
                        <div className="space-y-2 border-b border-slate-100 pb-2">
                            <div className="flex flex-col sm:flex-row sm:items-center">
                                ...
                                <Link href={`/contacts/${contact?.id || doc?.contact_id || ''}`} className="font-medium text-slate-800 cursor-pointer hover:bg-slate-100 px-2 py-1 rounded -ml-2 transition-colors self-start">
                        ...
                        <div className="space-y-2 border-b border-slate-100 pb-2">
                            <div className="flex flex-col sm:flex-row sm:items-center">
                                <span className="font-bold text-slate-700 sm:w-32 shrink-0">Generación:</span>
                                <span>{doc.issue_date || ''}</span>
                            </div>
                            <div className="flex flex-col sm:flex-row sm:items-center">
                                <span className="font-bold text-slate-700 sm:w-32 shrink-0">Vencimiento:</span>
                                <span>{doc.due_date || doc.issue_date || ''}</span>
                            </div>
                            {doc.cuds && (
                                <div className="flex flex-col sm:flex-row sm:items-start">
                                    <span className="font-bold text-slate-700 sm:w-32 shrink-0">CUDS:</span>
                                    <span className="break-all text-xs">{doc.cuds}</span>
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            <TableBody>
                                ...
                                {resolvedLines.length === 0 && (
                                    <TableRow className="hover:bg-transparent">
                                        <TableCell colSpan={8} className="h-24 text-center text-slate-400 border-l border-r border-gray-200">No hay productos en este documento</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
                        ...
                        <div className="w-full md:w-1/3 space-y-6 text-right flex flex-col justify-end">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span>$ {Number(doc.tax_total || 0).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center font-bold text-slate-800 pt-3 border-t border-slate-200">
                                    ...
                                    <span className="text-2xl">$ {Number(doc.payable_amount || 0).toLocaleString()}</span>
                                </div>
                                ...
                                ) : Number(doc.withholding_total) > 0 && (
                                    <div className="flex justify-between mt-2 pt-2 border-t border-slate-100">
                                        <span className="text-slate-500">Retenciones</span>
                                        <span className="text-red-500">-$ {Number(doc.withholding_total).toLocaleString()}</span>
                                    </div>
                                )}
                                <div className="flex justify-between font-bold text-slate-800 mt-2">
                                    ...
                                    <span className="text-xl text-green-600">$ {Number(doc.paid_amount || 0).toLocaleString()}</span>
                                </div>
                        ...
                        <div className="w-full md:w-1/2 space-y-10">
                            <div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex flex-col sm:flex-row border-b border-slate-100 pb-2">
                                        ...
                                        <span className="sm:w-1/2 font-medium">Documento soporte</span>
                                    </div>
                                    <div className="flex flex-col sm:flex-row border-b border-slate-100 pb-2">
                                        <span className="sm:w-1/2 font-bold text-slate-700">Indicador de procedencia</span>
                                        <span className="sm:w-1/2 font-medium">{doc.seller_origin_code === '11' ? 'No residente' : 'Residente'}</span>
                                    </div>
                                    {doc.order_reference_id && (
                                        <div className="flex flex-col sm:flex-row border-b border-slate-100 pb-2">
                                            <span className="sm:w-1/2 font-bold text-slate-700">Orden de compra</span>
                                            <span className="sm:w-1/2 font-medium">{doc.order_reference_id}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-xs text-black leading-relaxed text-justify">
                                    Este documento soporte se genera de conformidad con la Resolución DIAN vigente, por compras realizadas a sujetos no obligados a expedir factura de venta.
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="mt-10 pt-6 border-t border-slate-100 text-xs text-black text-center">
                        <p>Resolución DIAN: {doc.resolution?.resolution_text || ''}</p>
                    </div>
                </div>
            </div>
        </div>
