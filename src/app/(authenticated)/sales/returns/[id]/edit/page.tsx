...
import { useCreditNote, useUpdateCreditNote } from "@/hooks/returns/useReturns";
...
export default function ReturnEditPage() {
    ...
    const { data, isLoading, isError } = useCreditNote(id ?? "", { poll: false });
    const updateCreditNote = useUpdateCreditNote();

    const creditNote: any = data?.data?.credit_note || data?.credit_note || data?.creditNote;
    ...
    useEffect(() => {
        if (creditNote) {
            setClientId(String(creditNote.customer?.id ?? ""));
            setNote(creditNote.note ?? "");
            setObservation(creditNote.observation ?? "");
        }
    }, [creditNote]);
    ...
    if (isLoading) {
        return <div className="py-10 text-center text-sm text-slate-500">Cargando nota crédito...</div>;
    }
    if (isError || !creditNote) {
        return <div className="py-10 text-center text-red-500">No se pudo cargar la nota crédito</div>;
    }

    const dianStatusName: string = (creditNote.dian_status_name || creditNote.dian_status?.name || "").toUpperCase();
        ...
        return (
            <div className="max-w-2xl mx-auto py-10 px-4 sm:px-6 text-center">
                <h1 className="text-xl sm:text-2xl font-semibold text-[#0F2843] mb-4">
                    Nota crédito {creditNote.prefix || ""}{creditNote.number || creditNote.id}
                </h1>
                <div className="mb-4 text-red-600">Esta nota crédito ya fue aprobada por la DIAN y no puede ser editada.</div>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast("Nota crédito actualizada", "success", "Éxito");
            router.push(`/sales/returns/${id}`);
        } catch (error: any) {
            const msg = error?.response?.data?.message || error?.data?.message || "No se pudo actualizar la nota crédito";
    ...
    return (
        <div className="max-w-2xl mx-auto py-6 sm:py-10 px-4 sm:px-6">
            <Link href={`/sales/returns/${id}`} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-primary transition-colors mb-4">
                <ArrowLeft className="w-4 h-4" /> Volver al detalle
            </Link>
            <h1 className="text-xl sm:text-2xl font-semibold text-[#0F2843] mb-6">
                Editar nota crédito {creditNote.prefix || ""}{creditNote.number || creditNote.id}
            </h1>
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex flex-col sm:flex-row gap-2 mt-6">
                    ...
                    <Link href={`/sales/returns/${id}`} className="w-full sm:w-auto">
