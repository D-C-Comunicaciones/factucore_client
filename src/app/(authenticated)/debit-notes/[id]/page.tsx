...
import { useDebitNote, useSendDebitNote } from "@/hooks/debitNotes/useDebitNotes";
...
import { DebitNoteDetailHeader } from "@/components/debit-notes/details/DebitNoteDetailHeader";
import { DebitNoteDetailSummary } from "@/components/debit-notes/details/DebitNoteDetailSummary";
import { DebitNoteDetailDocument } from "@/components/debit-notes/details/DebitNoteDetailDocument";
import { DebitNoteDetailTabs } from "@/components/debit-notes/details/DebitNoteDetailTabs";
import { DebitNoteDetailSkeleton } from "@/components/debit-notes/details/DebitNoteDetailSkeleton";
...
export default function DebitNoteDetailPage() {
    ...
    const { data, isLoading, isError, isFetching, refetch } = useDebitNote(enabled ? id : "", { poll: pollEnabled });
    const sendToDian = useSendDebitNote();
    ...
    // El backend aún no habilita "debit_note" como tipo comentable (ver CommentableType
    // en types/comment.ts) — mientras tanto se usa el panel local (no persistido) para
    // conservar la misma estructura visual que el resto de documentos.
    ...
    const debitNoteData = data?.data?.debit_note || data?.debit_note || data?.debitNote;

    // El envío a la DIAN es asíncrono: mientras esté QUEUED/PROCESSING seguimos
    // haciendo polling (vía useDebitNote) hasta resolver o cumplir el timeout de UI.
    const dianSubmissionStatus = debitNoteData?.dian_submission_status;
    ...
    useEffect(() => {
        if (debitNoteData) {
            const docName = `Nota Débito No ${debitNoteData.prefix || ""}${debitNoteData.number || debitNoteData.id}`;
            document.title = docName;
        }
    }, [debitNoteData]);

    if (!enabled) return <div className="py-10 text-center text-red-500">ID de nota débito inválido</div>;
    if (isLoading || isRefreshing) return <DebitNoteDetailSkeleton />;
    if (isError || !data) return <div className="py-10 text-center text-red-500">No se pudo cargar la nota débito</div>;
    if (!debitNoteData) return <div className="py-10 text-center text-red-500">No se pudo cargar la nota débito</div>;

    const debitNote = debitNoteData;

    // Obtener data de la empresa
    const storedCompany = AuthService.getCompany<any>() || {};
    const invTplData = debitNote?.invoice?.invoice_snapshot?.template_data || {};
    const apiCompany = debitNote?.company || invTplData?.supplier_snapshot || invTplData?.supplier || {};
    const company = {
        ...storedCompany,
        ...apiCompany,
    ...
    const dianStatusName: string = (
        debitNote.dian_status_name ||
        debitNote.dian_status?.name ||
        ""
    ).toUpperCase();
    ...
    const handleSendToDian = async () => {
        ...
        try {
            ...
            // polling en useDebitNote mientras dian_submission_status esté QUEUED/PROCESSING.
            const res: any = await sendToDian.mutateAsync(debitNote.id);
            showToast(res?.message || "Nota débito creada. Estamos validándola ante la DIAN.", "success", "Éxito");
            resetPollTimeout();
            setPollEnabled(true);
        } catch (error: any) {
            const errorData = error.response?.data || error.data || error;
            const errorMsg = errorData?.message || "Error al emitir nota débito";
    ...
    const handlePrint = async () => {
        try {
            setIsPrinting(true);
            const blob = await DebitNotesService.printPdfBlob(debitNote.id);
            const docName = `Nota Débito No. ${debitNote.prefix || ""}${debitNote.number || debitNote.id}`;
    ...
    const handleDownloadPdf = async () => {
        try {
            ...
            const blob = await DebitNotesService.downloadPdfBlob(debitNote.id);
            const docName = `Nota Débito No. ${debitNote.prefix || ""}${debitNote.number || debitNote.id}.pdf`;
    ...
    const handleDownloadXml = async () => {
        try {
            ...
            const blob = await DebitNotesService.downloadXmlBlob(debitNote.id);
            const docName = `Nota Débito No. ${debitNote.prefix || ""}${debitNote.number || debitNote.id}.xml`;
    ...
    return (
        <div className="max-w-[1200px] mx-auto py-8 px-4 space-y-6 text-sm text-slate-700 relative">
            ...
            <DebitNoteDetailSummary debitNote={debitNote} />
            ...
            {isSubmissionPending || isSubmissionFailed ? (
                <DianSubmissionPendingCard
                    ...
                    documentLabel={`la nota débito No. ${debitNote.prefix || ''}${debitNote.number || debitNote.id}`}
                    onCheckNow={() => {
                        resetPollTimeout();
                        setPollEnabled(true);
                        refetch();
            ...
            <DebitNoteDetailDocument
                debitNote={debitNote}
                company={company}
                dianStatus={dianStatusName}
            />

            <DebitNoteDetailTabs debitNote={debitNote} />
