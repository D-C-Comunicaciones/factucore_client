...
export function useTrashedContactsList(options?: { params?: Record<string, any>; enabled?: boolean; fetchKey?: number }) {
    const paramsKey = JSON.stringify(options?.params ?? {});
    const page = options?.params?.current_page ?? 1;
    const perPage = options?.params?.per_page ?? 10;
    const fetchKey = options?.fetchKey ?? 0;
    ...
    return useQuery<ApiResponse<any>, Error, any>({
        queryKey: ["contacts", "trashed", paramsKey, page, perPage, fetchKey],

        queryFn: async () => {
            const res = await ContactsService.trashed(options?.params);

            if (!res || res.status !== "success") {
                throw new Error(res?.message || "Error al obtener la papelera de contactos");
            }
